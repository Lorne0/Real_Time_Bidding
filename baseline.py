...
Budget = [2, 4, 8, 16, 32]
# Constant
with open('./test/'+camp+'/result_all.txt', 'a') as fw:
    if algo == 'const':
        for B in Budget:
            clist = list(range(0, 401, 2))
            # Training Tune parameter
            tune_list = []
            for b in clist:
                budget = int(train_cost/len(train_log)*len(train_log)/B)
                cost = 0
                clks = 0
                for i in range(len(train_log)):
            ...
            # Testing
            budget = int(train_cost/len(train_log)*len(test_log)/B)
            cost = 0
            clks = 0
            for i in range(len(test_log)):
        ...
        for B in Budget:
            rlist = list(range(20, 100, 10)) + list(range(100, 401, 30))
            # Training Tune parameter
            tune_list = []
            for r in rlist:
                budget = int(train_cost/len(train_log)*len(train_log)/B)
        ...
        for B in Budget:
            B0 = list(range(2, 20, 2)) + list(range(20, 100, 5)) + list(range(100, 401, 10))
            # Training Tune parameter
            tune_list = []
            for b0 in B0:
                budget = int(train_cost/len(train_log)*len(train_log)/B)
